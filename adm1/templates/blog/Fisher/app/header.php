  ...
  function view_cat ($dataset) {
    ...
    foreach ($dataset as $menu) {
      ...
      if ( $menu['parent_id'] == 0 and empty($menu['childs']) ) {
        ...
        $link = '<a href="catalogue.php?id='.$menu['id'].'">'.$menu["name"].'</a>';
      }
...
<div class="header_wrapp">
    ...
    <ul>
      <li class="time">
        <div class="wrapp">
          ...
          <div class="content">
            <span>{{ header.daily_schedule }}</span>
          </div>
        </div>
      </li>
      <li class="email">
        <div class="wrapp">
          ...
          <div class="content">
            <a href="mailto:<?php print($config['email']); ?>"><{{ header.mail }}</a>
          </div>
        </div>
      </li>
      <li class="phone">
      ...
      <li class="social-networks">
        ...
        while ( $row = mysqli_fetch_assoc($result) ) {
          ...
          if ( !file_exists('img/icons/'.$row['link_image_light']) or empty($row['link_image_light']) ) {
            $row['link_image_light'] = 'img/items/template.png';
            $row['name_alt']   =  'Изображение временно отсутствует';
          } else {
            $row['link_image_light'] = 'img/icons/'.$row['link_image_light'];
            ...
            <div class="wrapp">
              <div class="img_wrapp">
                <a href="<?php print($row['link']); ?>">
                  <img src="{% static 'adm1/templates/blog/Fisher/app/img/logo.png' %}" alt="<?php print($row['name_alt']); ?>">
                </a>
              </div>
            </div>
          <?php
  ...
  <header class="header">
    <div class="container-header">
        <div class="logo">
          <div class="img_wrapp">
            <a href="index.php"><img src="<?php print($config['logo_path']); ?>" alt="Logo"></a>
          </div>
          <a href="index.php"><span class="logo_name"><?php print($config['logo_name']); ?></span></a>
        </div>
        <nav class="menu">
          <ul class="menu_ul">
    ...
    <div class="search">
      <div class="choice">
        ...
        <?php get_link('', 'Продажа', 'blue search-choice', 'data-type="default"', 'button'); ?>
        <?php get_link('', 'Прокат', 'blue search-choice', 'data-type="rent"', 'button'); ?>
      </div>
      <form>
      ...
      <div class="main"></div>
      <div class="links">
        <?php get_link('search.php', 'все результаты', 'blue'); ?>
        <?php get_link('search.php', 'новый поиск', 'black'); ?>
      </div>
    </div>
  </header>
</div>
